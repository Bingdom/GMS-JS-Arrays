...
///@arg {Function} callbackFn A function to call iteratively, accepting a previous value, a current value, a position value, and the originally supplied array.
...
function array_reduce(array, callbackFn, initialValue) {
	...
	for(var i= !is_undefined(initialValue) ? 0 : 1; i<array_length(array); i++) {
		previousValue = callbackFn(previousValue, array[i], i, array);
	}
